...
class AStarData:
    # YOUR CODE HERE
    def __init__(self, start_state, foods_coord):
        self.current = None
        self.destinations = foods_coord
        self.discovered = util.PriorityQueue()
        h = astar_heuristic(start_state.src, foods_coord)
...
def astar_loop_body(problem: q1b_problem, astarData: AStarData):
    ...
    for successor in problem.getSuccessors(astarData.current):
        ...
        if s_state.discovered is False:
            ...
            astarData.discovered.push(s_state, (s_state.distance + h, h))
        elif s_state.visited is False:
            if s_state.distance > astarData.current.distance + cost:
                s_state.distance = astarData.current.distance + cost
                s_state.previous = (astarData.current, action)
                astarData.discovered.update(s_state, (s_state.distance + h, h)) 
